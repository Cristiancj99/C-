...
void BusquedaInsercion ( int vector [], int util_vector){
	...
	for (int izqda=1; izqda< util_vector; izqda++){
		...
		valor= vector[izqda]; // tomo el valor a insertar en la parte ordenada
		
		for (i=izqda;( (i > 0 )&& (valor < vector[i-1]) ); i--){ // busco la posición donde me toca insertar el valor y desplazo los elementos hasta encotrar mi lugar

			vector [i]=vector [i-1]; // voy colandome hasta que encuentro mi posición final
		}
	vector [i]=valor;
	}
}
